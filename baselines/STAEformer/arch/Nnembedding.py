...
class NnEmbeddingFunction(nn.Module):
    """
    ...
    def __init__(
        ...
        if adaptive_embedding_dim > 0:
            ...
            #self.adaptive_embedding = nn.Embedding(self.num_nodes, self.adaptive_embedding_dim)
        ...
        if use_mixed_proj:
            self.output_proj = nn.Linear(
                in_steps * (self.model_dim - self.adaptive_embedding_dim + self.node_dim), out_steps * output_dim
            )
        else:
        ...
        self.attn_out = nn.ModuleList(
            [
                SelfAttentionLayer(self.model_dim - self.adaptive_embedding_dim + self.node_dim, out_feed_forward_dim, num_heads,
                                   dropout)
        ...
        self.fusion_model = nn.Sequential(
            *[MultiLayerPerceptron(input_dim=self.adaptive_embedding_dim + 2 * self.node_dim,
                                   ...
                                   dropout=0.2)
              for _ in range(2)],
            nn.Linear(in_features=self.adaptive_embedding_dim + 2 * self.node_dim, out_features=self.node_dim, bias=True)
        )
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, train: bool,
        ...
        if self.node_dim > 0:
            ...
            # for attn in self.attn_adj:
            #     graph = attn(graph, dim=2)
        ...
        if self.use_mixed_proj:
            ...
            out = out.reshape(
                batch_size, self.num_nodes, self.in_steps * (self.model_dim - self.adaptive_embedding_dim +self.node_dim)
            )
